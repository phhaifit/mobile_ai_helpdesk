...
abstract class _CustomerStore with Store {
  ...
  void setSearchQuery(String query) {
    searchQuery = query;
    // loadCustomers() is now handled by the debounced reaction!
  }
  ...
  Future<bool> saveCustomer(Customer customer) async {
    ...
    try {
      if (customers.any((c) => c.id == customer.id)) {
        ...
        // Validate email uniqueness internally
      ...
      if (e.response?.statusCode == 403) {
        ...
        errorMessage = e.response?.data?['message'] ?? e.message ?? 'Failed to save customer';
      }
      return false;
    } catch (e) {
