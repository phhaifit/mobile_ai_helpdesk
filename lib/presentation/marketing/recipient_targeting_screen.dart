...
class RecipientTargetingScreen extends StatefulWidget {
  ...
  @override
  State<RecipientTargetingScreen> createState() => _RecipientTargetingScreenState();
}
...
class _RecipientTargetingScreenState extends State<RecipientTargetingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l.translate('marketing_tv_recipient_targeting'))),
      body: SingleChildScrollView(
        ...
        child: Observer(
          builder: (_) => Column(
            ...
            children: [
              const Text('Loại bộ lọc', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const SizedBox(height: 8),
              ...RecipientFilterType.values.map((f) => Card(
                color: _store.draftFilterType == f ? Theme.of(context).colorScheme.primaryContainer : null,
                    ...
                    child: Row(
                      children: [
                        ...
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(l.translate('marketing_tv_estimated_count'), style: TextStyle(color: Colors.blue.shade700, fontSize: 12)),
                            Text('${_store.draftEstimatedCount}', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.blue.shade700)),
                          ],
                        ),
                      ],
                    ),
                ...
                child: FilledButton.icon(
                  onPressed: () {
                    final result = CampaignRecipientTarget(
                      ...
                      segmentValue: _store.draftSegmentValue.isEmpty ? null : _store.draftSegmentValue,
    ...
    switch (_store.draftFilterType) {
      case RecipientFilterType.tag:
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Chọn tag:', style: TextStyle(fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            Wrap(
              spacing: 8,
              runSpacing: 4,
              children: ['vip', 'regular', 'new', 'inactive', 'premium', 'loyal'].map((tag) {
      ...
      case RecipientFilterType.segment:
        return DropdownButtonFormField<String>(
          ...
          decoration: const InputDecoration(labelText: 'Phân khúc khách hàng', border: OutlineInputBorder()),
          items: const [
      ...
      case RecipientFilterType.channel:
        return DropdownButtonFormField<CampaignChannel>(
          ...
          decoration: const InputDecoration(labelText: 'Kênh liên lạc', border: OutlineInputBorder()),
  ...
  String _filterDescription(RecipientFilterType f) {
    switch (f) {
      case RecipientFilterType.all: return 'Gửi đến toàn bộ khách hàng';
      case RecipientFilterType.tag: return 'Lọc theo tag đã gán cho khách hàng';
      case RecipientFilterType.segment: return 'Lọc theo phân khúc khách hàng';
      case RecipientFilterType.channel: return 'Lọc theo kênh liên lạc';
    }
  }
