...
class _CampaignCreateScreenState extends State<CampaignCreateScreen> {
  ...
  void initState() {
    ...
    _nameController.addListener(() => _store.setDraftCampaignName(_nameController.text));
  }
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Observer(builder: (_) {
                switch (_currentStep) {
                  case 0: return _buildBasicInfoStep(l);
                  case 1: return _buildAudienceStep(l);
                  case 2: return _buildScheduleStep(l);
          ...
          return Expanded(
            child: Row(
              children: [
                CircleAvatar(
                  radius: 12,
                  backgroundColor: isDone ? Colors.green : (isActive ? Theme.of(context).colorScheme.primary : Colors.grey.shade300),
                  child: isDone
                      ? const Icon(Icons.check, size: 14, color: Colors.white)
                      : Text('${i + 1}', style: TextStyle(fontSize: 12, color: isActive ? Colors.white : Colors.grey.shade600)),
                ),
                const SizedBox(width: 4),
                Expanded(child: Text(steps[i], style: TextStyle(fontSize: 12, fontWeight: isActive ? FontWeight.bold : FontWeight.normal, color: isActive ? Theme.of(context).colorScheme.primary : Colors.grey.shade600))),
                if (i < steps.length - 1) Expanded(child: Divider(color: Colors.grey.shade300)),
              ],
            ),
          );
        }),
  ...
  Widget _buildBasicInfoStep(AppLocalizations l) {
    return Column(
      ...
      children: [
        Text(l.translate('marketing_tv_campaign_name'), style: const TextStyle(fontWeight: FontWeight.bold)),
        ...
        const SizedBox(height: 8),
        Observer(builder: (_) => DropdownButtonFormField<CampaignChannel>(
          value: _store.draftChannel,
          decoration: const InputDecoration(border: OutlineInputBorder(), prefixIcon: Icon(Icons.send_outlined)),
          items: CampaignChannel.values.map((ch) => DropdownMenuItem(value: ch, child: Text(_channelLabel(ch, l)))).toList(),
          onChanged: (v) { if (v != null) _store.setDraftChannel(v); },
        )),
        const SizedBox(height: 16),
        Text(l.translate('marketing_tv_template'), style: const TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Observer(builder: (_) {
          ...
          return Column(
            children: _store.templates.map((t) {
              final isSelected = _store.draftTemplateId == t.id;
              return Card(
                color: isSelected ? Theme.of(context).colorScheme.primaryContainer : null,
                child: ListTile(
                  leading: Icon(Icons.article_outlined, color: isSelected ? Theme.of(context).colorScheme.primary : Colors.grey),
                  title: Text(t.name),
  ...
  Widget _buildAudienceStep(AppLocalizations l) {
    return Observer(builder: (_) => Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Loại bộ lọc', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        ...RecipientFilterType.values.map((f) => RadioListTile<RecipientFilterType>(
          value: f,
          groupValue: _store.draftFilterType,
          title: Text(_filterLabel(f, l)),
          onChanged: (v) { if (v != null) _store.setDraftFilterType(v); },
          contentPadding: EdgeInsets.zero,
        )),
        if (_store.draftFilterType == RecipientFilterType.tag) ...[
          const SizedBox(height: 8),
          const Text('Chọn tag:', style: TextStyle(fontWeight: FontWeight.w500)),
          const SizedBox(height: 6),
          Wrap(
            spacing: 8,
            children: ['vip', 'regular', 'new', 'inactive', 'premium'].map((tag) {
        ...
        if (_store.draftFilterType == RecipientFilterType.segment) ...[
          ...
          DropdownButtonFormField<String>(
            value: _store.draftSegmentValue.isEmpty ? null : _store.draftSegmentValue,
            decoration: const InputDecoration(labelText: 'Phân khúc', border: OutlineInputBorder()),
            items: const [
              DropdownMenuItem(value: 'VIP', child: Text('VIP')),
              DropdownMenuItem(value: 'Regular', child: Text('Regular')),
              DropdownMenuItem(value: 'New', child: Text('New')),
            ],
            onChanged: (v) { if (v != null) _store.setDraftSegmentValue(v); },
          ),
        ],
        if (_store.draftFilterType == RecipientFilterType.channel) ...[
          ...
          DropdownButtonFormField<CampaignChannel>(
            ...
            decoration: const InputDecoration(labelText: 'Kênh', border: OutlineInputBorder()),
            items: CampaignChannel.values.map((ch) => DropdownMenuItem(value: ch, child: Text(_channelLabel(ch, l)))).toList(),
            onChanged: (v) { if (v != null) _store.setDraftChannelFilter(v); },
          ),
        ],
        const SizedBox(height: 16),
        SizedBox(
          ...
          child: OutlinedButton.icon(
            onPressed: _store.isSubmitting ? null : _store.estimateAudience,
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text('${l.translate('marketing_tv_estimated_count')}: ', style: const TextStyle(fontWeight: FontWeight.w500)),
                  Text('${_store.draftEstimatedCount}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.blue.shade700)),
                ],
              ),
            ),
          ),
      ...
      children: [
        ...
        if (_store.draftScheduledAt != null)
          Padding(
            ...
            child: Text('Lên lịch: ${_formatDate(_store.draftScheduledAt!)}', style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.w500)),
          ),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              children: [
                _summaryRow('Tên', _store.draftCampaignName.isEmpty ? '(chưa nhập)' : _store.draftCampaignName),
                const SizedBox(height: 6),
                _summaryRow('Kênh', _channelLabel(_store.draftChannel, l)),
                const SizedBox(height: 6),
                _summaryRow('Template', _store.draftTemplateId.isEmpty ? '(chưa chọn)' : _store.draftTemplateId),
                const SizedBox(height: 6),
                _summaryRow('Đối tượng', _filterLabel(_store.draftFilterType, l)),
                if (_store.draftEstimatedCount > 0) ...[
                  const SizedBox(height: 6),
                  _summaryRow('Ước tính', '${_store.draftEstimatedCount} người'),
                ],
                const SizedBox(height: 6),
                _summaryRow('Gửi', _store.draftScheduledAt != null ? _formatDate(_store.draftScheduledAt!) : 'Ngay sau khi tạo'),
              ],
            ),
          ),
        ),
                ...
                : FilledButton(
                    onPressed: _store.isSubmitting
                        ...
                        : () async {
                            ...
                            if (mounted && _store.actionWasSuccess) Navigator.pop(context);
                          },
                    child: _store.isSubmitting
                        ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                        : Text(l.translate('marketing_btn_create_campaign')),
                  ),
          ),
        ],
      )),
  ...
  String _filterLabel(RecipientFilterType f, AppLocalizations l) {
    switch (f) {
      case RecipientFilterType.all: return l.translate('marketing_tv_filter_all');
      case RecipientFilterType.tag: return l.translate('marketing_tv_filter_tag');
      case RecipientFilterType.segment: return l.translate('marketing_tv_filter_segment');
      case RecipientFilterType.channel: return l.translate('marketing_tv_filter_channel');
    }
  }
