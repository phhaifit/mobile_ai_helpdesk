...
class AccountApi {
  ...
  Future<List<TeamMember>> getTenantMembers() async {
    ...
    return items
        .map((item) {
          final id =
              (item['accountID'] ?? item['accountId'] ?? item['id']) as String?;
          final tenantId =
              (item['tenantID'] ?? item['tenantId'] ?? '') as String?;
          final email = (item['email'] ?? '') as String;
          final displayName =
              (item['fullname'] ?? item['displayName']) as String?;
          final roleRaw = (item['role'] ?? '') as String? ?? '';
          final roleStr = roleRaw.toLowerCase();
          final role =
              roleStr == 'admin' || roleStr == 'administrator'
                  ? TeamRole.admin
                  : TeamRole.customerSupport;
          // API doesn't return permission objects — populate an empty list here.
          final permissions = <dynamic>[];
          final isBlocked = item['isBlocked'];
          final isActive = isBlocked is! bool || !isBlocked;
          ...
          final avatarUrl =
              (item['profilePicture'] ?? item['avatarUrl']) as String? ?? '';
          ...
          return TeamMember(
            ...
            permissions: List<Permission>.from(
              permissions.whereType<Map<String, dynamic>>().map(
                Permission.fromJson,
              ),
            ),
            isActive: isActive,
